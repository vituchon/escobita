...
func AdquireWebSocket(w http.ResponseWriter, r *http.Request) {
	...
	if !isNew {
		msg := fmt.Sprintf("Web socket already adquired for client(id='%d')", services.GetClientPlayerId(r))
		log.Println(msg)
		http.Error(w, msg, http.StatusBadRequest)
	} else {
		log.Printf("Web socket(RemoteAddr='%s') adquired OK (connection \"upgraded\") for client(id='%d')\n", conn.RemoteAddr().String(), services.GetClientPlayerId(r))
	}
}

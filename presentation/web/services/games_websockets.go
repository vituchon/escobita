...
func (gws *gameWebSockets) BindClientWebSocketToGame(response http.ResponseWriter, request *http.Request, gameId int) {
	log.Printf("Binding web socket from client(id=%d) in game(id=%d)...", GetClientPlayerId(request), gameId)
	...
	for _, exstingConn := range gws.ConnsByGameId[gameId] {
		if exstingConn == conn {
			msg := fmt.Sprintf("Web socket(remoteAddr='%s') from client(id=%d) already binded in game(id=%d)", conn.RemoteAddr().String(), GetClientPlayerId(request), gameId)
			log.Println(msg)
	...
	log.Printf("Binded web socket(remoteAddr='%s') from client(id=%d) in game(id=%d)", conn.RemoteAddr().String(), GetClientPlayerId(request), gameId)
}
